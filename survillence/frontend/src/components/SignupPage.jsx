...
import { setUserRole } from '../redux/userAction'; // Ensure this path matches your project structure
    ...
    >
      ...
      <div className="absolute top-0 right-0 mt-2 mr-4 text-xs">
        <select className="bg-transparent w-full backdrop-blur-2xl text-white rounded p-2 ">
          <option value="en" className="text-black">English (UK)</option>
        </select>
      </div>
      <div
        className="max-w-lg w-full p-16 flex flex-col gap-2 mt-1 rounded-xl shadow-md relative"
        style={{
      ...
      >
        ...
        <div className="py-4 mt-4 flex flex-col gap-4">
          <button
            onClick={() => handleRoleSelection('surveillancePartner')}
          ...
          <button
            onClick={() => handleRoleSelection('safetyAlertsUser')}
            className="w-full p-2 text-center bg-gradient-to-r text-sm from-blue-700 to-sky-400 text-white rounded-lg shadow-sm hover:from-blue-600 hover:to-sky-500 duration-1000"
          >
